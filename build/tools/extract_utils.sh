...
function setup_vendor() {
    ...
    export AWAKEN_ROOT="$3"
    if [ ! -d "$AWAKEN_ROOT" ]; then
        echo "\$AWAKEN_ROOT must be set and valid before including this script!"
        exit 1
    fi

    export OUTDIR=vendor/"$VENDOR"/"$DEVICE"
    if [ ! -d "$AWAKEN_ROOT/$OUTDIR" ]; then
        mkdir -p "$AWAKEN_ROOT/$OUTDIR"
    fi
    ...
    export PRODUCTMK="$AWAKEN_ROOT"/"$OUTDIR"/"$VNDNAME"-vendor.mk
    export ANDROIDBP="$AWAKEN_ROOT"/"$OUTDIR"/Android.bp
    export ANDROIDMK="$AWAKEN_ROOT"/"$OUTDIR"/Android.mk
    export BOARDMK="$AWAKEN_ROOT"/"$OUTDIR"/BoardConfigVendor.mk
...
function write_blueprint_header() {
    ...
    if [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
        printf " * Copyright (C) $YEAR The Project-Awaken\n" >> $1
    elif [ $BLUEPRINT_INITIAL_COPYRIGHT_YEAR -le 2019 ]; then
        printf " * Copyright (C) 2019-$YEAR The Project-Awaken\n" >> $1
    else
        printf " * Copyright (C) $BLUEPRINT_INITIAL_COPYRIGHT_YEAR-$YEAR The Project-Awaken\n" >> $1
    fi
...
function write_makefile_header() {
    ...
    if [[ $INITIAL_COPYRIGHT_YEAR =~ $NUM_REGEX ]] && [ $INITIAL_COPYRIGHT_YEAR -le $YEAR ]; then
        ...
        if [ $YEAR -eq 2017 ]; then
            printf "# Copyright (C) 2017 The Project-Awaken\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -eq $YEAR ]; then
            printf "# Copyright (C) $YEAR The Project-Awaken\n" >> $1
        elif [ $INITIAL_COPYRIGHT_YEAR -le 2017 ]; then
            printf "# Copyright (C) 2017-$YEAR The Project-Awaken\n" >> $1
        else
            printf "# Copyright (C) $INITIAL_COPYRIGHT_YEAR-$YEAR The Project-Awaken\n" >> $1
        fi
    else
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$AWAKEN_ROOT"/prebuilts/tools-awaken/common/smali/baksmali.jar
        export SMALIJAR="$AWAKEN_ROOT"/prebuilts/tools-awaken/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$AWAKEN_ROOT"/prebuilts/tools-awaken/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$AWAKEN_ROOT"/prebuilts/tools-awaken/${HOST}-x86/bin/compact_dex_converter
    fi
    ...
    if [ ! -f "$AWAKEN_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$AWAKEN_TARGET" >/dev/null; then
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$AWAKEN_ROOT"/vendor/lineage/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$AWAKEN_ROOT"/"$OUTDIR"/radio
